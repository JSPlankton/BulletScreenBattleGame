...
public class DataTable : Dictionary<string, SingleData>
{
                ...
                if (line[i] != "" && line[i] != null)
                {
                    ...
                    for (int j = 0; j < data.TableKeys.Count; j++)
                    {
                        ...
                        if (!row[j].Equals(""))
                        {
                            ...
                            dataTmp.Add(debugProperty, row[j]);
                        }
                    }
    ...
    public void AddItem(params string[] datas)
    {
        SingleData dataTmp = new SingleData();
        dataTmp.data = this;
        for (int i = 0; i < this.TableKeys.Count && i < datas.Length; i++)
        {
            dataTmp.Add(this.TableKeys[i], datas[i]);
        }
        this.AddData(dataTmp);
    }
    ...
    public static string Serialize(DataTable data)
    {
        ...
        //LogUtil.Log("type count " + type.Count);
    ...
    public static string[] ConvertStringArray(string lineContent)
    {
        ...
        for (int i = 0; i < lineContent.Length; i++)
        {
            if (state)
            {
                if (lineContent[i] == c_split)
                {
                    ...
                    result[0] = lineContent.Substring(startIndex, i - startIndex);
                    startIndex = i + 1;
                    break;
                }
        ...
        result[1] = lineContent.Substring(startIndex, lineContent.Length - startIndex);
    ...
    public void AddData(SingleData data)
    {
        if (data.ContainsKey(TableKeys[0]))
        {
            ...
            Add(data.m_SingleDataKey, data);
            TableIDs.Add(data.m_SingleDataKey);
        }
        else
        {
