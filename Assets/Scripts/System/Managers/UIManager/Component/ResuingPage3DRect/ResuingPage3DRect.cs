...
public class ResuingPage3DRect : ScrollRectInput
{
    public string m_ItemName = "";

    public float scrollVelocity = 1f;      //page的滑动速度
    public float paddingSpaceHead = 0;
    public float paddingSpaceTail = 0;
    ...
    public void ReSetData(List<Dictionary<string, object>> data)
    {
        if (m_items.Count > 0)
        {
            ClearDatas();
    ...
    void SetItemDisplay(bool isRebuild = false)
    {
        ...
        {
            m_items[i].OnDrag();
            m_items[i].OnUpdateItem(m_viewBounds, GetItemBounds(m_items[i].m_index));
    ...
    #region Page相关
    public void ShowPage(int pageNum, float speed = 4.0f)
    {
        if (horizontal)
        {
            this.horizontalNormalizedPosition = -0.5f;
        }
        m_smooting = speed;
    ...
    public void JumpPage(int pageNum)
    {
        ...
        {
            EventManager.DispatchEvent(this, "PageRect_PageChange_3D", new object[] { m_curPageNum });
        }
    ...
    void SetPageSyn()
    {
        ...
        posDiff += ((posDiff - m_startDragPos) * m_sensitivity);
